# Vendored from torchreid (github.com/KaiyangZhou/deep-person-reid), MIT License,
# Copyright (c) 2018 Kaiyang Zhou. See NOTICE.md. Unmodified except this header.
import warnings
...
class ConvLayer(nn.Module):
    ...
    def __init__(
        self, in_channels, out_channels, kernel_size, stride=1, padding=0, groups=1, IN=False
    ):
        super().__init__()
        self.conv = nn.Conv2d(
            in_channels,
...
class Conv3x3(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, groups=1):
        super().__init__()
        self.conv = nn.Conv2d(
            in_channels, out_channels, 3, stride=stride, padding=1, bias=False, groups=groups
        )
        self.bn = nn.BatchNorm2d(out_channels)
        self.relu = nn.ReLU(inplace=True)
